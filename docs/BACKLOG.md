...
**Done:** `services/small_router.py` now estimates prompt size before calling qwen3-router and falls back to `manual_agent_required` with reason `router_context_budget_exceeded` when the prompt exceeds the safe input budget or when llama.cpp still returns a context-limit error. Defaults are `SMALL_ROUTER_BASE_URL=http://localhost:11430/v1`, `SMALL_ROUTER_MODEL=qwen3-router`, `SMALL_ROUTER_TIMEOUT=10.0`, `SMALL_ROUTER_MAX_CONTEXT=2048`, `SMALL_ROUTER_SAFE_INPUT_TOKENS=1200`, and `SMALL_ROUTER_MAX_OUTPUT_TOKENS=128`. Router output remains advisory only (`authority: false`) and cannot approve or execute; explicit manual agent selection bypasses the tiny router. Regression: `python scripts/test_small_router.py` / `scripts/test_router_authority_boundary.ps1`.
