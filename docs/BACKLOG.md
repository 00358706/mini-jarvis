...
**Done:** `services/small_router.py` now estimates prompt size before calling qwen3-router and falls back to `manual_agent_required` with reason `router_context_budget_exceeded` when the prompt exceeds the safe input budget or when llama.cpp still returns a context-limit error. `/plans/from-message` may omit an explicit agent; the local dashboard defaults to Automatic and uses the tiny router as an advisory route selector only, falling back to manual selection when qwen3-router is unavailable, over budget, invalid JSON, low confidence, unknown route, or route-without-agent. Defaults are `SMALL_ROUTER_BASE_URL=http://localhost:11430/v1`, `SMALL_ROUTER_MODEL=qwen3-router`, `SMALL_ROUTER_TIMEOUT=10.0`, `SMALL_ROUTER_MIN_CONFIDENCE=0.60`, `SMALL_ROUTER_MAX_CONTEXT=2048`, `SMALL_ROUTER_SAFE_INPUT_TOKENS=1200`, and `SMALL_ROUTER_MAX_OUTPUT_TOKENS=128`. Router output remains advisory only (`authority: false`) and cannot approve or execute; explicit manual agent selection bypasses the tiny router. Regression: `python scripts/test_small_router.py`, `python scripts/test_chatbar_router_control_surface.py`, `python scripts/test_dashboard_chatbar_static.py`, / `scripts/test_router_authority_boundary.ps1`.
