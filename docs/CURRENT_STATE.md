...
- **Tiny router is advisory and context-budgeted**: `services/small_router.py` can ask qwen3-router to choose an agent only for short prompts. `SMALL_ROUTER_MAX_CONTEXT` (default `2048`), `SMALL_ROUTER_SAFE_INPUT_TOKENS` (`1200`), and `SMALL_ROUTER_MAX_OUTPUT_TOKENS` (`128`) keep requests away from the llama.cpp context limit; oversized prompts or context-limit errors return `manual_agent_required` with reason `router_context_budget_exceeded`. Router results keep `authority: false` and never approve or execute.
...
- **Plan API**
  - `POST /plans/propose` — persists pending plans with a server-computed **`reviewed_plan_sha256`** over the canonical plan core (client-supplied digest fields are stripped and recomputed). Legacy on-disk plans without hashes are intentionally unsupported for approve/execute until re-proposed. Each **step** may include nested **`safety`** metadata (idempotency / dry-run intent / compensation notes); fields are **evidence for review** — the gateway does **not** auto-run rollback or compensation, and **`dry_run`** does not change execute behavior yet.
  - `GET /plans/pending`
...
- `python scripts/test_small_router.py` / `scripts/test_router_authority_boundary.ps1` — qwen3-router context-budget guard: short prompts call once, oversized prompts do not call the model, llama.cpp context errors become `manual_agent_required`, router output remains advisory (`authority: false`), and manual agent selection bypasses the tiny router.
