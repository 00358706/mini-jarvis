# Read-only automation lab review summary CLI wrapper.
...
if (($RequestId -and $RequestId.Trim()) -eq ($Path -and $Path.Trim())) {
    throw "Provide exactly one of -RequestId or -Path."
}

$RepoRoot = Resolve-RepoRoot
$Py = Select-Python -RepoRoot $RepoRoot
$ArgsList = @(
    (Join-Path $RepoRoot "automation_lab_review.py")
)

if ($RequestId -and $RequestId.Trim()) {
    $ArgsList += @("--request-id", $RequestId.Trim())
} else {
    $ArgsList += @("--path", $Path.Trim())
}
