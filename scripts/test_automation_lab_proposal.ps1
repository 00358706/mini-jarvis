...
try {
    ...
    Assert-True ($capabilities.schema_version -eq "automation-lab-capability-matches.v3") "CAPABILITY_MATCHES.json should use v3 schema with scoring and registry evidence."
    Assert-True ($null -ne $capabilities.score) "Advisory score must be present."
    Assert-True ($null -ne $capabilities.score_breakdown) "score_breakdown must be present."
    Assert-True ($null -ne $capabilities.precedence_applied) "precedence_applied must be present."
